	...
	public abstract class BaseWeapon : Item, IWeapon, IScissorable, ICraftable, ISlayer, IDurability
	{
		...
		public virtual double ScaleDamageAOS( Mobile attacker, double damage, bool checkSkills )
		{
			...
			if ( damageBonus > 100 )
				damageBonus = 100;
			#endregion
			if (attacker is PlayerMobile && ((PlayerMobile)attacker).Barbarian() )
			{
	                        double totalBonus = strengthBonus + (anatomyBonus * 1.2) + (tacticsBonus * 1.5)  + necroBonus + wizardBonus + bowyerBonus + bushidoBonus + ninjaBonus + armsLoreBonus + lumberBonus + miningBonus + fishingBonus + ((double)(GetDamageBonus() + damageBonus) / 100.0);
