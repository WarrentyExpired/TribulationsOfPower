...
namespace Server.Items
{
	...
	{
        #region Auto Bandage Self

        public static class AutoBandage
        {
            private static readonly object m_AutoBandageRunning = new object();
            ...
            public static void Configure()
            {
                ...
                CommandSystem.Register("AutoBandage", AccessLevel.Player, HandleCommand);
            }
            ...
            private static void HandleCommand(CommandEventArgs e)
            {
                if (Enabled)
                {
                    if (StartAutoBandage(e.Mobile, TimeSpan.FromSeconds(1), true))
                    {
                        e.Mobile.SendMessage("Auto-bandage has been started!");
                    }
                    else if (StopAutoBandage(e.Mobile))
                    {
                        e.Mobile.SendMessage("Auto-bandage has been stopped!");
                    }
                }
            }
            ...
            {
                StopAutoBandage(e.Mobile);
            }

            private static bool StopAutoBandage(Mobile mob)
            {
                if (!mob.CanBeginAction(m_AutoBandageRunning))
                {
                    mob.EndAction(m_AutoBandageRunning);
            ...
            private static bool StartAutoBandage(Mobile mob, TimeSpan delay, bool checkAction)
            {
                if (!checkAction || mob.BeginAction(m_AutoBandageRunning))
                {
                    Timer.DelayCall(delay, new TimerStateCallback(OnAutoBandage), new object[] { mob, delay });
            ...
            private static void OnAutoBandage(object state)
            {
                var states = (object[])state;

                OnAutoBandage((Mobile)states[0], (TimeSpan)states[1]);
            }

            private static void OnAutoBandage(Mobile mob, TimeSpan delay)
            {
                if (mob.CanBeginAction(m_AutoBandageRunning))
                {
                    return;
                ...
                {
                    var bandage = mob.Backpack.FindItemByType<Bandage>();

                    if (bandage != null)
                    {
                        mob.RevealingAction();

                        var target = new InternalTarget(bandage);

                        target.Invoke(mob, mob);

                        var context = BandageContext.GetContext(mob);

                        if (context != null && context.Timer != null)
                        {
                            StartAutoBandage(mob, context.Timer.Delay, false);
                            return;
                        }
                    }
                }

                StartAutoBandage(mob, delay, false);
            }
        }
	...
	public class BandageContext
	{
		...
		public static double HealTimer( Mobile healer, Mobile patient )
		{
			...
			if ( healer == patient )
				if (healer is PlayerMobile && ((PlayerMobile)healer).Barbarian() )
				{
					seconds = 3;
				}
				else
